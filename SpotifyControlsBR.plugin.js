 ...
 * @author DevilBro (original), santozrx (tradução/adaptação)
 * @authorId 930664055650287706
 * @version 1.5.1
 * @description Tradução em português feita por santozrx baseada na versão original 1.5.0 de DevilBro
 * @invite Jx3TjNS
 * @donate https://www.paypal.me/MircoWittrien
 * @patreon https://www.patreon.com/MircoWittrien
 * @website https://mwittrien.github.io/
 * @source https://github.com/mwittrien/BetterDiscordAddons/tree/master/Plugins/SpotifyControls/
 * @updateUrl https://raw.githubusercontent.com/Santozrlk/SpotifyControls/main/SpotifyControls.plugin.js
 */
...
module.exports = (_ => {
	...
	children: [
		...
		BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.TextElement, {
			...
			children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.TextScroller, {
				children: _this.settings.general.addBy
					? (() => {
						const artistas = lastSong.state.split(";").map(n => n.trim());
						return "Por: " + artistas.join(", ");
					  })()
					: lastSong.state
			})
		})
		...
		return class SpotifyControls extends Plugin {
			onLoad () {
				...
				this.defaults = {
general: {
	addBy:             {value: true,  description: "Adiciona a palavra 'por' antes do nome do autor"},
	addTimeline:       {value: true,  description: "Mostra a linha do tempo da música nos controles"},
	addActivityButton: {value: true,  description: "Exibe o botão de alternância de status de atividade nos controles"},
	hideDisabled:      {value: false, description: "Oculta botões que não podem ser clicados (ex: volume no celular ou botão anterior sem Premium)"},
	doubleBack:        {value: true,  description: "Exige que o usuário pressione o botão de voltar duas vezes para ir para a faixa anterior"}
},
buttons: {
	share:     {value: {small: false, big: true}, icons: [""],            description: "Compartilhar"},
	shuffle:   {value: {small: false, big: true}, icons: [""],            description: "Aleatório"},
	previous:  {value: {small: true,  big: true}, icons: [""],            description: "Anterior"},
	pauseplay: {value: {small: true,  big: true}, icons: ["", ""],       description: "Pausar/Tocar"},
	next:      {value: {small: true,  big: true}, icons: [""],            description: "Próxima"},
	repeat:    {value: {small: false, big: true}, icons: ["", ""],       description: "Repetir"},
			...
			onStart () {
				...
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.InternalReactUtils, ["jsx", "jsxs"], {before: e => {
					if (e.methodArguments[0] == "section" && e.methodArguments[1].className && e.methodArguments[1].className.indexOf(BDFDB.disCN.channelpanels) > -1) {
						...
						if (parent) parent.props.children.unshift(BDFDB.ReactUtils.createElement(SpotifyControlsComponent, {
